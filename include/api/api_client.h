...
uint32 CountRegisteredCallbacks(int iCallbackId) {
  ...
  if (s_bDispatcherInited) {
    ...
    if (!pDisp->m_CallbackMap.empty()) {
      for (auto it = pDisp->m_CallbackMap.begin();
           it != pDisp->m_CallbackMap.end(); ++it) {
...
static void WarnMissingInterface(HSteamPipe hPipe, const char *iface) {
  ...
  g_pfnNotifyMissing =
      (Fn_NotifyMissing)GetProcAddress(hMod, "Steam_NotifyMissingInterface");
...
static void SetAppIDEnv() {
  ...
  _snprintf_s(szGame, sizeof(szGame), _TRUNCATE, "%llu",
              CGameID(g_ForcedAppId).ToUint64());

  uint32 overlayAppId =
      (g_OriginalAppId != 0) ? g_OriginalAppId : g_ForcedAppId;
  _snprintf_s(szOverlayGame, sizeof(szOverlayGame), _TRUNCATE, "%llu",
              CGameID(overlayAppId).ToUint64());
...
static void WriteAppIDFile() {
  ...
  if (GetModuleFileNameA(nullptr, exePath, MAX_PATH) != 0) {
    if (PathRemoveFileSpecA(exePath)) {
      ...
      _snprintf_s(fullPath, sizeof(fullPath), _TRUNCATE, "%s\\steam_appid.txt",
                  exePath);
...
S_API const char *S_CALLTYPE SteamAPI_GetSteamInstallPath() {
  ...
  LSTATUS GetPID = GetRegistryDWORD("Software\\Valve\\Steam\\ActiveProcess",
                                    "pid", ActiveProcessPID);

  if (GetPID == ERROR_SUCCESS && ActiveProcessPID != 0) {
    HANDLE hPIDProcess =
        OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, FALSE, ActiveProcessPID);
...
#if defined(_M_IX86)
        GetDllString =
            GetRegistryString("Software\\Valve\\Steam\\ActiveProcess",
                              "SteamClientDll", g_InstallPath, MAX_PATH);
#elif defined(_M_AMD64)
        GetDllString =
            GetRegistryString("Software\\Valve\\Steam\\ActiveProcess",
                              "SteamClientDll64", g_InstallPath, MAX_PATH);
#endif
        ...
        if (GetDllString == ERROR_SUCCESS) {
          ...
          if (FixPath == TRUE) {
            ...
            UCOColor(FOREGROUND_RED | FOREGROUND_INTENSITY,
                     "[UCOnline2] PathRemoveFileSpecA Failed "
                     "(SteamAPI_GetSteamInstallPath)!\r\n");
          }
        } else {
          UCOColor(FOREGROUND_RED | FOREGROUND_INTENSITY,
                   "[UCOnline2] Unable to get steamclient(64).dll path "
                   "(SteamAPI_GetSteamInstallPath)!\r\n");
        }
      } else {
        UCOColor(FOREGROUND_RED | FOREGROUND_INTENSITY,
                 "[UCOnline2] GetExitCodeProcess Failed "
                 "(SteamAPI_GetSteamInstallPath)!\r\n");
      }
    } else {
      UCOColor(
          FOREGROUND_RED | FOREGROUND_INTENSITY,
          "[UCOnline2] OpenProcess Failed (SteamAPI_GetSteamInstallPath)!\r\n");
    }
  } else {
    UCOColor(FOREGROUND_RED | FOREGROUND_INTENSITY,
             "[UCOnline2] Unable to get the PID of the Steam process "
             "(SteamAPI_GetSteamInstallPath)!\r\n");
  }
...
SteamInternal_SteamAPI_Init(const char *pszVersions, SteamErrMsg *pOutErr) {
  ...
  if (g_pSteamClient) {
    UCOColor(FOREGROUND_RED | FOREGROUND_INTENSITY,
             "[UCOnline2] Init already called, use Shutdown first\r\n");
    return k_ESteamAPIInitResult_OK;
  }

  g_pSteamClient = static_cast<ISteamClient *>(InitSteamClient(
      &g_ClientModule, s_bAnonUser, STEAMCLIENT_INTERFACE_VERSION));

  if (!g_pSteamClient) {
    UCOColor(FOREGROUND_RED | FOREGROUND_INTENSITY,
             "[UCOnline2] Failed to get SteamClient\r\n");
    SteamAPI_Shutdown();
  ...
  if (!s_bAnonUser) {
    ...
    if (g_ClientPipe == 0) {
      UCOColor(FOREGROUND_RED | FOREGROUND_INTENSITY,
               "[UCOnline2] CreateSteamPipe failed\r\n");
      SteamAPI_Shutdown();
    ...
    g_ClientUser =
        g_pSteamClient->CreateLocalUser(&g_ClientPipe, k_EAccountTypeAnonUser);
  ...
  if (g_ClientUser != 0) {
    if (pszVersions) {
      ...
      g_pfnIsKnownInterface =
          (Fn_IsKnownInterface)GetProcAddress(hMod, "Steam_IsKnownInterface");
    ...
    ISteamUtils *pUtils = (ISteamUtils *)g_pSteamClient->GetISteamUtils(
        g_ClientPipe, STEAMUTILS_INTERFACE_VERSION);
    if (pUtils) {
      uint32 reportedID = pUtils->GetAppID();

      if (reportedID == 0)
        UCOColor(FOREGROUND_RED | FOREGROUND_INTENSITY,
                 "[UCOnline2] Steam reported AppID 0, forcing 480\r\n");
      ...
      g_pSteamClient->Set_SteamAPI_CCheckCallbackRegisteredInProcess(
          CountRegisteredCallbacks);

      ISteamUser *pUser = (ISteamUser *)g_pSteamClient->GetISteamUser(
          g_ClientUser, g_ClientPipe, STEAMUSER_INTERFACE_VERSION);
      if (pUser) {
      ...
      if (!g_bClientReady) {
        UCOColor(FOREGROUND_RED | FOREGROUND_INTENSITY,
                 "[UCOnline2] g_ClientCtx.Init failed\r\n");
        SteamAPI_Shutdown();
        return k_ESteamAPIInitResult_VersionMismatch;
      }
      ...
      UCOColor(FOREGROUND_RED | FOREGROUND_INTENSITY,
               "[UCOnline2] Failed to get ISteamUtils\r\n");
    ...
    UCOColor(FOREGROUND_RED | FOREGROUND_INTENSITY,
             "[UCOnline2] Failed to connect/create user\r\n");
    SteamAPI_Shutdown();
    return k_ESteamAPIInitResult_NoSteamClient;
  }
...
S_API bool S_CALLTYPE SteamAPI_InitAnonymousUser() {
  s_bAnonUser = true;
  bool result =
      SteamInternal_SteamAPI_Init(nullptr, nullptr) == k_ESteamAPIInitResult_OK;
  s_bAnonUser = false;
  return result;
}
...
S_API bool S_CALLTYPE SteamAPI_InitSafe() {
  ...
  UCOColor(FOREGROUND_RED | FOREGROUND_INTENSITY,
           "[UCOnline2] SteamAPI_InitSafe failed\r\n");
  return false;
}
...
S_API bool S_CALLTYPE SteamAPI_IsSteamRunning() {
  ...
  if (GetPID == ERROR_SUCCESS && ActiveProcessPID != 0) {
    ...
    if (hPIDProcess != nullptr) {
      ...
      UCOColor(FOREGROUND_RED | FOREGROUND_INTENSITY,
               "[UCOnline2] OpenProcess Failed (SteamAPI_IsSteamRunning)!\r\n");
    }
  } else {
    UCOColor(FOREGROUND_RED | FOREGROUND_INTENSITY,
             "[UCOnline2] Unable to get the PID of the Steam process "
